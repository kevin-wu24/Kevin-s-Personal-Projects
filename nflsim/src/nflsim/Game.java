...
public class Game {
    ...
    public String avgGameWithEstSpread(Team Home, Team Away, int WeekNum) throws Exception
    {
    	...
    	{
    		return homeName + " " + Math.round((1.0/(double)WeekNum)*homeLast+Math.log(WeekNum)/Math.log(4)*Home.calcPoints()+3*Home.calcEffWinPct()+1.5) + "     " + awayName + " " + Math.round((1.0/(double)WeekNum)*awayLast+Math.log(WeekNum)/Math.log(4)*Away.calcPoints())+ "\n -" + Math.round((Math.max((1.0/(double)WeekNum)*homeLast+Math.log(WeekNum)/Math.log(4)*Home.calcPoints()+3*Home.calcEffWinPct()+1.5,(1.0/(double)WeekNum)*awayLast+Math.log(WeekNum)/Math.log(4)*Away.calcPoints()) - Math.min((1.0/(double)WeekNum)*homeLast+Math.log(WeekNum)/Math.log(4)*Home.calcPoints()+3*Home.calcEffWinPct()+1.5,(1.0/(double)WeekNum)*awayLast+Math.log(WeekNum)/Math.log(4)*Away.calcPoints()))*2.0)/2.0;
    	}
